...
if(!empty(array_filter($_POST))) {
	...
	// keep the existing stage because it can't be edited in this interface
	$stage = $data["Stage"];
	// read the values out of the HTML
	...
	// the data array that will be written into the second row in the csv 
	$values = [
		$stage,
		$numberOfStudents,
		$monday,
...
foreach(read("csv/projekte.csv") as $p){
  for($i = 5; $i < 12; $i++) {
  ...
  Auswählen, wieviele Projekte gewählt werden soll. Mit Formel Empfehlung abgeben, auch aufgrund der verfügbaren Plätze?? Erst wenn Projektwahl-Stage eingestellt wird -->
	...
	<div class="container">
		<div class="card-deck">
		  ...
		  <div class="card text-white bg-dark mb-3" >
					<div class="card-body">
					<h5 class="card-title" id="eingereichteProjekte"><?php echo count(read('csv/projekte.csv')); ?></h5>
...
<div class="container">
	<div class="progress" style= "height: 4em;" >
		<div class="progress-bar bg-dark"
			role="progressbar"
			style="width: 25%;"
			aria-valuenow="<?php echo count(read('csv/schueler.csv')); ?>"
			aria-valuemin="0"
			aria-valuemax="1000">
			<?php echo(count(read('csv/schueler.csv')))/1000*100  ?> %
		</div>
	</div>
	<p> <?php echo count(read('csv/schueler.csv'));?> von ### Schülern haben bereits gewählt</p>
</div> -->
